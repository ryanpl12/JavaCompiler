/* 
    ...
    In this file I have added operations such as node handling, traversal, and printouts displaying the AST created by the CMINIS+ code.
...
void ASTprint(int level, ASTnode *p)
{
    ...
    switch (p->type)
        ...
        case A_VARDEC:
           ...
           if(p -> value > 0)  
           { 
            // For array variables, print the variable type, name, and size
            printf("Variable %s %s with array size [%d] \n", ASTtypeToString(p -> my_data_type), p->name, p -> value);
           } else {  
            // For simple variables, print the variable type and name
        ...
        case A_FUNCTIONDEC:
            PT(level); 
            // Print the function's return type, name, and indicate the start of its definition
            printf("Function %s %s \n", ASTtypeToString(p -> my_data_type), p -> name); 
        ...
        case A_PARAM: 
            ...
            if (p->value != 0)
                {
                printf("[]"); // If value is not zero, it indicates an array parameter,print []
                }
            printf("\n");
            if (p->next != NULL)
                {
        ...
        case A_VAR:
            ...
            if(p -> s1 == NULL) 
            {
                ...
                printf("VARIABLE %s ", p -> name); // print the variables name
                PT(level + 1);
                printf("\n");
                PT(level);
                printf("[\n"); // Open the array
